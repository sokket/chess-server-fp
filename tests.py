...
def create_pair():
    ...
    client.send(invite_code.encode("ascii"))
...
class TestServer(unittest.TestCase):
    ...
    def test_ping_pong(self):
        ...
        for i in range(0, 100):
            send_byte(host, PKG_PING)
            self.assertEqual(p_byte(client.recvn(1)), PKG_PING)
            send_byte(client, PKG_PONG)
            self.assertEqual(p_byte(host.recvn(1)), PKG_PONG)
        client.close()
        host.close()
    ...
    def test_chat(self):
        ...
        self.assertEqual(client.recvuntil('\0'.encode("ascii")), message)
        client.close()
        host.close()
